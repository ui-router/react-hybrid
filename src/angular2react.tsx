...
export function angular2react<Props extends object>(
  ...
  return class Angular2React extends React.Component<Props, State<Props>> {
    ...
    render() {
      ...
      if (component.bindings) {
        for (const binding in component.bindings) {
          const isStringBinding = /@/.exec(component.bindings[binding]);

          if (isStringBinding) {
            const type = typeof this.props[binding];
            if (type !== 'undefined' && type !== 'string') {
              throw new Error(`Expected a string value when binding to '${binding}' attribute of the angular2react component '${componentName}'`);
            }
